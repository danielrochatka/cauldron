...
This factory owns the wiring that assembles a ``ContentOperationService``
from ``settings.CAULDRON_MODULES`` — including the ContentRouter, the
workspace change-set store, the snapshot service, and the FlatFile
reversible-mutation adapter when the routing selects the ``flatfile``
provider.

Ownership: this module lives in ``cauldron_content_operations`` because it
is the module that provides the service. Historically the factory lived in
``cauldron_admin_content.service_factory``; that path is preserved as a
compatibility re-export so downstream callers do not need to change their
imports.

Callers that want a ContentOperationService should import from here:
